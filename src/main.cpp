...
void refresh_interactive_line(const std::string& prompt,
  ...
  const std::size_t width = prompt.size() + line.size();
...
void usage() {
  std::cerr << "usage: fluxion <check|emit-llvm|run> <file.flx>\n"
...
class ReplSession {
 public:
  bool handle(const std::string& input) {
    ...
    if (text == ":decls") {
      std::cout << (declarations_.empty() ? "(none)\n" : declarations_);
      return true;
    }
 ...
 private:
  ...
  static void print_help() {
    std::cout << "Fluxion REPL commands:\n"
              << "  :help   show this help\n"
              << "  :decls  show persistent declarations\n"
              << "  :clear  clear persistent declarations\n"
              << "  :quit   exit\n"
              << "\n"
              << "Enter expressions to evaluate, or struct/func declarations to keep in the session.\n"
              << "Line editing: Up/Down history, Ctrl-A start, Ctrl-E end, Ctrl-K kill to end.\n";
  }
...
int run_repl(std::istream& input, bool interactive) {
  ...
  if (interactive) {
    std::cout << "Fluxion REPL. Type :help for commands, :quit to exit.\n";
  }

  for (;;) {
    if (interactive) {
      auto maybe_line = read_interactive_line(buffer.empty() ? "fluxion> " : "......> ", history);
      if (!maybe_line) {
        return 0;
      }
      line = *maybe_line;
